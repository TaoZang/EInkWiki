...
2. 更新变更说明，并以规范三段版本号创建标签，例如 `v0.3.0`。
3. 推送提交和标签：`git push origin main && git push origin v0.3.0`。
4. GitHub Actions 会运行 lint、单元测试、debug/release 构建、zipalign 和签名验证。
5. 成功后 Release 会包含且只依赖以下更新资产：
   - `EInkWiki-v0.3.0.apk`
   - `EInkWiki-v0.3.0.apk.sha256`
...
```bash
./gradlew -PversionName=0.3.0 -PversionCode=7 lintRelease testReleaseUnitTest assembleRelease
```
