...
public final class ZimArchive implements Closeable {
    public static final String CONTENT_ORIGIN = "https://zim.local/";
    private static final String TAG = "ZimArchive";
    ...
    private ZimArchive(Context context, File file) throws Exception {
    ...
    public static ZimArchive open(Context context, File file) throws Exception {
        if (file == null || !file.isFile()) {
            throw new IOException("离线包不存在");
        }
        return new ZimArchive(context, file);
    }

    public static void validate(Context context, File file) throws Exception {
        NativeRuntime.ensureLoaded(context);
    ...
    public static ZimBook inspect(Context context, File file) throws Exception {
        NativeRuntime.ensureLoaded(context);
        Archive candidate = null;
        try {
            candidate = new Archive(file.getCanonicalPath());
            ...
            String title;
            try {
                title = candidate.getMetadata("Title");
            } catch (Exception ignored) {
                title = "";
            }
            return new ZimBook(
                    ...
                    title,
                    file.length(),
                    Math.max(0, candidate.getArticleCount())
            );
        } finally {
            if (candidate != null) {
                candidate.dispose();
    ...
    /** Returns random readable articles from the currently open local archive. */
    public synchronized List<SearchResult> randomEntries(int count) {
        requireOpen();
        int safeCount = Math.max(1, Math.min(10, count));
        List<SearchResult> result = new ArrayList<>(safeCount);
        Set<String> paths = new HashSet<>();
        int attempts = safeCount * 40;
        while (result.size() < safeCount && attempts-- > 0) {
            try {
                Entry entry = archive.getRandomEntry();
                Item item = entry.getItem(true);
                String path = item.getPath();
                if ("text/html".equals(normalizedMime(item.getMimetype()))
                        && path != null
                        && !path.isEmpty()
                        && paths.add(path)) {
                    result.add(new SearchResult(item.getTitle(), path, ""));
                }
            } catch (Exception ignored) {
                // A random entry can be a damaged redirect; try another entry.
            }
        }
        return result;
    }
